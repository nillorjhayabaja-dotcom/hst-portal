...
function BarChartInner({ data, height = 200, showLabels = true, className }: { data: DataPoint[]; height?: number; showLabels?: boolean; className?: string }) {
...
function LineChartInner({ data, height = 200, className }: { data: DataPoint[]; height?: number; className?: string }) {
  const maxValue = Math.max(...data.map((d) => d.value), 1);
  ...
  return (
    <div className={cn("relative", className)} style={{ height }}>
      <svg viewBox={`0 0 ${width} 100`} className="h-full w-full overflow-visible" preserveAspectRatio="none">
      ...
      <div className="mt-1 flex justify-between px-0">
        {data.filter((_, i) => i % Math.ceil(data.length / 6) === 0 || i === data.length - 1).map((d, i) => (
          <span key={i} className="text-[9px] text-muted-foreground/70">{d.label}</span>
        ))}
      </div>
    </div>
  );
...
function PieChartInner({ data, size = 160, className }: { data: DataPoint[]; size?: number; className?: string }) {
  ...
  return (
    <div className={cn("flex items-center gap-4", className)}>
      <svg viewBox="-1 -1 2 2" className="shrink-0" style={{ width: size, height: size }}>
        {slices.map((slice, i) => (
          <path key={i} d={slice.path} fill={slice.color} stroke="hsl(var(--background))" strokeWidth="0.02" />
        ))}
      </svg>
...
function AreaChartInner({ data, height = 200, className }: { data: DataPoint[]; height?: number; className?: string }) {
  const maxValue = Math.max(...data.map((d) => d.value), 1);
  const width = 100;
  const stepX = width / (data.length - 1);
  ...
  return (
    <div className={cn("relative", className)} style={{ height }}>
      <svg viewBox={`0 0 ${width} 100`} className="h-full w-full overflow-visible" preserveAspectRatio="none">
        ...
        <path d={lineD} fill="none" stroke="hsl(var(--primary))" strokeWidth="2" vectorEffect="non-scaling-stroke" />
      </svg>
...
export function ChartCard({ title, subtitle, children, loading, className, height, action }: ChartCardProps) {
  return (
...
interface BarChartProps { data: DataPoint[]; height?: number; showLabels?: boolean; className?: string; loading?: boolean; title?: string; subtitle?: string; }
export function BarChart({ data, height, showLabels, className, loading, title, subtitle }: BarChartProps) {
  if (title || subtitle) {
    return (
  ...
  return <BarChartInner data={data} height={height} showLabels={showLabels} className={className} />;
}

interface LineChartProps { data: DataPoint[]; height?: number; className?: string; loading?: boolean; title?: string; subtitle?: string; }
export function LineChart({ data, height, className, loading, title, subtitle }: LineChartProps) {
  ...
  if (title || subtitle) return <ChartCard title={title ?? ""} subtitle={subtitle} loading={loading} height={height}>{content}</ChartCard>;
  if (loading) return <ChartSkeleton height={height} />;
  return content;
}

interface PieChartProps { data: DataPoint[]; size?: number; className?: string; loading?: boolean; title?: string; subtitle?: string; }
export function PieChart({ data, size, className, loading, title, subtitle }: PieChartProps) {
  ...
  if (title || subtitle) return <ChartCard title={title ?? ""} subtitle={subtitle} loading={loading}>{content}</ChartCard>;
  if (loading) return <ChartSkeleton height={size ?? 160} />;
  return content;
}

interface AreaChartProps { data: DataPoint[]; height?: number; className?: string; loading?: boolean; title?: string; subtitle?: string; }
...
interface MultiSeriesChartProps { series: Series[]; height?: number; className?: string; loading?: boolean; title?: string; subtitle?: string; }
export function MultiBarChart({ series, height = 200, className, loading, title, subtitle }: MultiSeriesChartProps) {
    ...
    <div className={cn("flex items-end gap-2", className)} style={{ height }}>
      {series[0]?.data.map((_, idx) => (
        <div key={idx} className="flex flex-1 flex-col items-center gap-1 h-full justify-end">
          ...
          <span className="text-[9px] text-muted-foreground/70">{series[0]?.data[idx]?.label ?? ""}</span>
        </div>
      ))}
    </div>
  );
